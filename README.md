...
- **Adversarial preprocessor (3)** ⚠ self-authored — synthetic perturbations (`adversarial-{typo,unicode,code}`) generated by `packages/eval/private/scripts/generate_adversarial_bench.py` over a project-curated PII pool (12 names, 6 emails, etc.) with project-chosen perturbations (single-char typos, Cyrillic homoglyphs, dash banners). Lift comes from `_normalize_for_detection` (NFKC + unidecode + zero-width strip + spaced-PII despace), which targets exactly the perturbation classes we generated. **Treat these scores as a regression test for the preprocessor, not a generalisation claim.**
