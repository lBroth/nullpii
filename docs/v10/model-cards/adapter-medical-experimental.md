...
The unified release bench does NOT include i2b2 / MEDDOCAN test (gating on validation completion). Numbers reported on `tab-echr` / `presidio-synthetic` / `nullpii-bench` reflect general-purpose performance, not clinical-domain performance.
