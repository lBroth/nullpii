...
python -u scripts/bench_full.py \
  --tools nullpii,nullpii-router-embedding,presidio,nemotron-pii-raw,piiranha,deberta,gliner-onnx-pii-fp32,gliner-pii-large-v1 \
...
(full). Single-tool re-run: `--tools nullpii-router-embedding`.

### Tool surface (8 rows)

`nullpii` (subprocess of the local npm build, the canonical user-facing
row) + `nullpii-router-embedding` (Python re-impl of the same
...
| `ai4privacy-300k-heldout` | HF `ai4privacy/pii-masking-300k` offset 100k+ | held-out non-adversarial |
| `isotonic-en-heldout` | HF `Isotonic/pii-masking-200k` offset 200k+ | held-out non-adversarial |
| `isotonic-de-heldout` | HF `Isotonic/pii-masking-200k` offset 200k+ | held-out non-adversarial |
